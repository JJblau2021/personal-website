  ...
  return (
    <Link href={app.path} className="block group">
      <Card className="h-full overflow-hidden rounded-2xl pixel-hover cursor-pointer border-2 border-transparent bg-card shadow-md hover:border-primary/30 dark:hover:border-primary/50 transition-all duration-300">
        <div className="relative w-full overflow-hidden bg-gradient-to-br from-primary/5 via-accent/5 to-secondary/5 flex items-center justify-center">
          ...
          {app.thumbnail ? (
            ...
            <div className="relative size-20 rounded-xl bg-primary/10 flex items-center justify-center border-2 border-primary/20 group-hover:scale-110 group-hover:rotate-3 transition-transform duration-300">
          ...
          <div className="absolute inset-0 bg-primary/25 backdrop-blur-sm opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center p-4">
            <p className="text-sm text-foreground text-center line-clamp-3 leading-relaxed">
              {app.description}
            </p>
          </div>
        </div>
        ...
        <div className="p-3">
          <div className="flex items-center justify-between gap-2 mb-2">
            ...
            <Badge
              variant="secondary"
              className={`text-xs shrink-0 rounded-lg border px-2 py-0.5 ${categoryColors[app.category]}`}
            >
              {categoryLabels[app.category]}
            </Badge>
          </div>

          <div className="flex items-center gap-1.5 text-sm font-medium text-primary group-hover:text-primary-light transition-colors">
